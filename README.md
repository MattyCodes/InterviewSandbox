...
- **Windows**: `cmd.exe` and PowerShell can't run these scripts directly. Use **Git Bash** (installed with [Git for Windows](https://git-scm.com/download/win), which also gives you `ssh` and `openssl`) — install [Multipass for Windows](https://multipass.run/install) separately, then run every `./interview-sandbox` command from a Git Bash terminal. WSL also works, but Multipass still has to be the native Windows build (it needs Hyper-V) — don't install it inside your WSL distro; WSL picks up `multipass.exe` from the Windows PATH on its own.
  - **Windows Home** (no Hyper-V) has to use Multipass's VirtualBox driver (`multipass set local.driver=virtualbox`) instead, which needs [VirtualBox](https://www.virtualbox.org/wiki/Downloads) installed separately. On some setups this only gives the VM a NAT adapter with no host-routable IP — if `up` fails with "Could not determine VM IP address", install [PsExec](https://learn.microsoft.com/en-us/sysinternals/downloads/pstools) and put `PsExec64.exe` on your PATH; the tool uses it to read the SSH port Multipass already forwards for its own use in that case, and connects through that instead.
